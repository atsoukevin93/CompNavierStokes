...
def Explicit_Staggered(rho, U, dt, dx):
    ...
    diag0=1 -dt/dx * (pplus(U[1: N-1])- pminus(U[0: N-1]))
    diagp= -dt/dx * (pminus(U[1:N-1]))
    diagm=dt/dx * (pplus(U[0: N-1]))
    ...
    B= 1/dt * (np.concatenate(shiftd(rho) + rho, (shiftd(rho)+ rho)[0]) * U
      -1/dx * np.concatenate([barotrope_rhotoP(Rho_new, c, gamma) -shiftd(barotrope_rhotoP(Rho_new, c, gamma)), [barotrope_rhotoP(Rho_new, c, gamma) -shifd(barotrope_rhotoP(Rho_new, c, gamma))[0]]])
